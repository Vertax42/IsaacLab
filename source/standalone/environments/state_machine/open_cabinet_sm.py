...
parser = argparse.ArgumentParser(
    description="Pick and lift state machine for cabinet environments."
)
parser.add_argument(
    "--disable_fabric",
    action="store_true",
...
from omni.isaac.lab_tasks.manager_based.manipulation.cabinet.cabinet_env_cfg import (
    CabinetEnvCfg,
)
...
def infer_state_machine(
    ...
    if state == OpenDrawerSmState.REST:
        ...
        des_ee_pose[tid] = wp.transform_multiply(
            handle_approach_offset[tid], handle_pose[tid]
        )
        gripper_state[tid] = GripperState.OPEN
        ...
        des_ee_pose[tid] = wp.transform_multiply(
            handle_grasp_offset[tid], handle_pose[tid]
        )
        ...
        des_ee_pose[tid] = wp.transform_multiply(
            drawer_opening_rate[tid], handle_pose[tid]
        )
        gripper_state[tid] = GripperState.CLOSE
        # wait for a while
...
class OpenDrawerSm:
    ...
    def __init__(
        ...
        self.handle_approach_offset = torch.zeros(
            (self.num_envs, 7), device=self.device
        )
        self.handle_approach_offset[:, 0] = -0.1
        self.handle_approach_offset[:, -1] = (
            1.0  # warp expects quaternion as (x, y, z, w)
        )
        ...
        self.handle_approach_offset_wp = wp.from_torch(
            self.handle_approach_offset, wp.transform
        )
        self.handle_grasp_offset_wp = wp.from_torch(
            self.handle_grasp_offset, wp.transform
        )
        self.drawer_opening_rate_wp = wp.from_torch(
            self.drawer_opening_rate, wp.transform
        )
...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            cabinet_position = (
                cabinet_frame_tf.data.target_pos_w[..., 0, :].clone()
                - env.unwrapped.scene.env_origins
            )
