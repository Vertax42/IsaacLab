...
class CommandManager(ManagerBase):
    ...
    def set_command(self, name: str, value: torch.Tensor):
        """Manually override the command for the specified term."""
        if name not in self._terms:
            raise KeyError(f"Command term '{name}' not found in the manager.")
        # 获取具体的命令项
        cmd_term = self._terms[name]
        # 设置命令
        cmd_term.command = value
        # print(f"[DEBUG] Command '{name}' set to: {value}")  # 添加调试信息
